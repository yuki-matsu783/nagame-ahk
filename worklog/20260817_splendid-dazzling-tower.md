...
- `grep -rn "flow-id \d+|全体フロー\d+ステップ|\d+ステップに対応"`による機械的な洗い出しと、
  事前に立てた「対象外」判定基準（DDR不変、issue-create/SKILL.mdのflow-id 1・2〜3は変更不要等）を
  照合する検証方法が、実装後の取りこぼし確認まで一貫して機能した（想定通り取りこぼし無し）。
- HANDOFF.mdのフロー進捗状況テーブルが「今動いているタスク自身の進捗追跡」と「次タスクへの
  リセット用テンプレート」の二重の役割を持つことに実装直前で気づき、33行化を次のリセット
  （flow-id 21→31）まで見送る判断ができた。plan通りに機械的に差し替えていた場合、進捗表示が
  実態と矛盾する状態になっていた。

## ダメだったこと

- `index.jsonl`（frontmatter抽出インデックス）の再生成を試みたが、`参考ディレクトリ/`を含む
  全体スキャンでタイムアウトし、一部ファイルが末尾の行を欠いたまま中断された状態で残った。
  対象を絞った個別実行では成功したはずが、その直後にスコープ外のファイル（`plans/index.jsonl`等）
  まで変更されており、ルート`index.jsonl`には同一内容の重複行が生じるなど、原因を特定できない
  挙動が発生した。plan本来のスコープ外でもあるため深追いせず全て`git checkout`でrevertした。
