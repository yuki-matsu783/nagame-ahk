...
| 19 | `plans/` `worklog/` を削除し、`HANDOFF.md` を次タスクへリセットする | エージェント |
| 20 | commit, push する | エージェント |
| 21 | マージする（squash merge。ブランチは削除してよい） | 人間 |
...
2. 返信本文を組み立てる。**AIエージェントが返信する場合は、本文の先頭に必ず
   `🤖 Claude Code (AI) より:` の署名行を付ける。** `gh` / `glab` CLIは人間のアカウントで認証
...
2. サブエージェントが返す「現在地サマリ」（ブランチ・issue・PR/MR・未解決コメント件数・
   ...
   ユーザーに提示する。
3. 提示した内容をもとに、全体フロー21ステップのうちどこから再開すべきかをAIエージェントが判断し、
