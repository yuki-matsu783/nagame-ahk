...
- flow-id 14〜15: ユーザーから「レビューOK」の合図を受けたが、`get_mr_unresolved_comments 29 true`
  で未解決スレッド1件（トークン数の既知の過小カウント問題をドキュメントへ反映する指摘）を検出し、
  対応・返信・push済み（commit `20f8b74`）。**ただしスレッドの解決（resolved）操作はレビュアー側の
  操作のため、返信しただけでは`unresolved`のまま**。ユーザーに、対応内容の確認とスレッドの
  解決操作（またはさらなるコメント）を依頼する必要がある。
