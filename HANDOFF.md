...
- flow-id 12〜13完了: 実装をcommit（fix/test/docsの3コミットに分割）・push、MR descriptionを
  実装状況で更新済み
- 「レビューOK」の連絡を受け、`comments all`（`get_mr_unresolved_comments 47 true`）で未解決の
  実装レビュースレッドが無いことを確認（自動投稿の工数レポートコメントのみ）。flow-id 14〜15完了
- flow-id 16完了: plan・worklogの内容を設計ドキュメントへ反映
  - `dev-tools/docs/spec/issue-mr-workflow.md`「対応工数レポート」節を新方式（セッション横断
    カーソル＋新規行diff集計・skillCalls/agentCalls/askUserQuestions詳細テーブル・`usage/`
    ディレクトリ移設）に合わせて全面更新。新規サブセクション「新規行diff方式への移行」
    「呼び出し・質問の詳細記録」を追加、「コンポーネント」の関数一覧を実装と一致するよう更新、
    「未決定事項・懸念点」に新規行diff方式の既知の限界（resumeによる物理的な重複行は除外しない）を追記
  - `dev-tools/docs/ddr/0006-対応工数レポートはtranscript自前パースで実装する.md`へ「追記
    （issue #37）」セクションを追加（マージ済みDDRのため既存内容は変更せず追記のみ。uuid重複排除案が
    却下された経緯・採用したセッション横断カーソル設計・既知の限界・`usage/`移設をまとめて記録）
