...
| [x][x][x][] | 11 | planをもとに作業を進める、作業内容はworklogに更新する | エージェント |
| [x][x][x][] | 12 | commit, push してレビュー依頼を行う | エージェント |
...
- `.claude/settings.json`のhook `command`は当初git bash本体のフルパスを直書きしていたが、
  レビュー往復の結果「`"bash"`のみ・PATH解決に依存する」方針に変更した。このマシンではPATHの
  優先順位次第でWSL側のbash.exeスタブへ黙って解決される問題があり、対処として**システム環境変数**
  （`Machine`スコープ、ユーザー環境変数では効果が無い）の`Path`にgit bashの`bin`を追加する
  セットアップが必要（`shell-scripts.md`「Claude Code hookの起動コマンド」に手順を記載。
  このマシンでは適用済みで動作確認済み）。
