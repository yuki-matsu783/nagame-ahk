...
- commit/push・PR description更新（`describe`）。人間から「レビューOK」を受け、
  `comments all`で未解決コメント無し（自動使用量レポートのみ）を確認済み（flow-id 7〜8完了）。
...
- 詳細は`plans/groovy-twirling-puffin.md`参照。compact後、この実装から再開する。
