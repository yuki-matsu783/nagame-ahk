...
| `test_usage_tracking.sh` | `.claude/hooks/lib/UsageTracking.sh`の純粋ロジック（`_usage_aggregate_transcript`のgapベースの稼働時間`activeSeconds`算出、`_usage_merge_state`の累計差分計算、`_usage_merge_agent_state`のサブエージェント`agentId`/`agentType`二段集計、`_usage_sync_session_logs`の`~/.claude/projects`模擬ツリーからのローカルコピー、`_usage_aggregate_and_merge_subagents`のエンドツーエンド集計） | あり（`$TMPDIR`配下に合成JSONLフィクスチャ・疑似ディレクトリツリーを作成・削除するのみ） | `bash tests/test_usage_tracking.sh`（git bash） |
